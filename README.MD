...
This project demonstrates the deployment of a web application using the LAMP stack (Linux, Apache, MySQL, PHP) on an AWS EC2 instance. The project is hosted on an Ubuntu 22.04 LTS server, and the infrastructure is designed to be accessible via the internet.

### Website Link

You can access the deployed application here: [Visit My Website](http://3.145.192.72/)

## Architecture Diagram

![Architecture Diagram](infra/architecture-diagram.drawio.png)


The above diagram illustrates the infrastructure setup:
- **Developer**: Develops and manages the codebase using GitHub for version control.
- **GitHub**: Stores the project's source code.
- **AWS Cloud**: Hosts the application within a Virtual Private Cloud (VPC) on an EC2 instance.
- **EC2 Instance**: Runs Ubuntu 22.04 LTS with Apache, MySQL, and PHP installed to host the web application.
- **Public Subnet**: Ensures the application is accessible from the internet.
- **Internet**: Allows users to access the application globally.

## Project Components

1. **Apache (Web Server)**:
   - Handles incoming HTTP requests and serves the PHP application.
   
2. **MySQL (Database)**:
   - Manages the application's data storage needs.
   
3. **PHP (Backend)**:
   - Powers the server-side logic of the application.

4. **Ubuntu 22.04 LTS**:
   - The operating system used on the EC2 instance to run the LAMP stack.
...
- **Basic knowledge of LAMP stack components**.
